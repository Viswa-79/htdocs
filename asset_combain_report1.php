<?php include "config.php"; ?>
 ...
 ?>
           ...
           <h1 class="btn btn-outline-secondary" style="font-size:20px">Credence Combain Report</h1>
...
<div class="row g-4">
    ...
    <table id="ConvertTable" class="table table-bordered table-hover ">
        <thead style="text-align:center;font-weight:bold">
                <tr style="background-color:#e5e5ff">
                  <th  colspan="9">Outward</th>
</tr>
    ...
    <table id="ConvertTable" class="table table-bordered table-hover ">
        ...
        <th   colspan="11">Inward</th>
                 ...
                 $sql4 = " SELECT *,s.id as id,s.date as date,a.name as name,s1.status as status,s.rec_no as rec_no FROM asset_inward s left join asset_inward1 s1 on s.id=s1.cid left join asset_master c on s1.ass_name=c.id left join asset_group j on s1.ass_grp=j.id left join partymaster a on s.supplier=a.id left join asset_assign b on s1.ass_no=b.id   where s.date between '$fromdate' and '$todate' $rr order by s1.id asc";
                 $result4 = mysqli_query($conn, $sql4);
                 while($wz1 = mysqli_fetch_array($result4))
                 {
                  $date=$wz1['date'];
...
<a href="asset_combain_report.php" class="btn btn-primary mt-4"><i class="ti ti-arrow-left me-sm-1 me-0"></i>Back</a>
