...
<script type="text/javascript" src="http://jqueryjs.googlecode.com/files/jquery-1.3.1.min.js" > </script> 
 ...
 <?php
error_reporting(0);
$sid=base64_encode($_GET['cid']);
$a1=mysqli_query($conn,"SELECT * FROM order1 where cid='$sid'");

$so=mysql_fetch_array($a1);
...
<?php
$a1=mysql_query("SELECT * FROM address");

$ad=mysql_fetch_array($a1);


$b1=mysql_query("SELECT * FROM partymaster where sno='$party'");

$pm=mysql_fetch_array($b1);
...
<body>
   ...
   <?php
       $b1="select * from salesorder1 where cid='$sid'  and quantity!='' ";
	$b2=mysql_query($b1);
	while($b3=mysql_fetch_object($b2))
	{ 
	if($b3->pono!='' && $b3->pono!='-')
	{
	$pono2[]=$b3->pono;
  ...
  <table width="878"  align="center" style="border-collapse:collapse; border:none;">
    ...
    <tr height="130px">
	  ...
	  Order No : OMT/<?php echo $so['invoiceno'];?></br>
      
      <?php if($ponos!='' && $so['partyref']=='')
	  { ?>
	  Po No : <?php echo $ponos;?>
      <?php } ?>
	  
	    <?php if($so['partyref']!='' && $so['partyref']!='-')
	  { ?>
	  Po No : <?php echo $so['partyref'];?></b>
      <?php } ?>
      </b>
</td>
	  </tr>
...
?>
   ...
   <table width="878"  align="center" style="border:none;margin-top:5px;">
	  ...
	  <u>For the attention of <?php echo $so['partycontactperson'];?></u>
	  </br></br>
	  Dear <?php echo $so['sir'];?>,
	   </br></br>
...
<?php
  ...
  <table width="875" align="center" style=" border:1px solid #000000; border-collapse: collapse;">
    <tr>
      ...
      <td width="99"  style=" border:1px solid #000000; "><div align="center"><strong>Price in </br><?php echo $so['currency'];?>&nbsp;<?php echo $so['shipmentterms'];?></strong></div></td>
      <td width="99"  style=" border:1px solid #000000; "><div align="center"><strong>Quantity <?php if($so['ordertype']=='MADEUPS'){ echo "Pcs"; } else {echo "Mtrs"; }?> (+/- <?php echo $so['tolerance'];?>%)</strong></div></td>
	  <td width="92"  style=" border:1px solid #000000; "><div align="center"><strong>Total <?php echo $so['currency'];?>&nbsp;<?php echo $so['shipmentterms'];?></strong></div></td>
...
$i=1;
  ...
  <tr style="border-bottom:1px solid black;" >
      ...
      <td height="21" style="border-right:1px solid black;vertical-align:top;"> <div align="left" style="margin-left:2%;"><strong><?php if($b3->pono!='' && $b3->pono!='-' && $count1>1){ ?>PO No. : <?php echo $b3->pono;echo "&nbsp;&nbsp;"; }?><?php if($b3->itemno!='' && $b3->itemno!='-'){ ?>Item no : <?php echo $b3->itemno;echo"</br>"; }?></strong>
	  <?php echo $b3->particulars;?>
	
	<?php if($b3->print!='' && $b3->print!='-' && $b3->print!='NO' && $b3->print!='NO PRINT')
	{ ?>  
 ...
 <?php
  if($b3->size!='' && $b3->size!='-'){ ?>     <strong>Size:</strong><?php echo $b3->size; }?>
      ...
      <td  style="border-right:1px solid black;" ><div align="center" style="margin-left:2%;"><?php echo $b3->quantity;?> <?php echo $b3->unit;?></div>      </td>
          <td  style="border-right:1px solid black;" ><div align="center" ><?php echo number_format((float)$b3->amt, 2, '.', ''); ?></div>      </td>
    </tr>
   <?php
...
<td style="border:none; border-right:none; vertical-align:text-top;" >:</td>
 ...
 <span class="style5"><?php echo $so['loading'];?></span>
</td>
...
<td style="border:none; border-right:none; vertical-align:text-top;" >:</td>
 ...
 <span class="style5"><?php echo $so['packing'];?></span>
</td>

 
 </tr>
 
 <?php if($so['ordertype']!='MADEUPS' && $so['piecelength']!='' && $so['piecelength']!='-' ){ ?>
...
<td style="border:none; border-right:none; vertical-align:text-top;" >:</td>
 ...
 <span class="style5"><?php echo $so['piecelength'];?></span>
</td>
...
<?php /*?>  <div align="center">
  ...
  <a href="salesorder_word.php?cid=<?php echo $sid;?>" >  <input type="button" value="Export To Word" /></a>
  </div><?php */?>
</body>
</html>
